    ...
    <div className="flex flex-col h-full bg-card/30 border border-border rounded-lg overflow-hidden shadow-inner">
      ...
      <div className="flex-1 overflow-auto p-4 font-mono text-sm">
        {history.map((line, i) => (
          <div
            key={i}
            className={`mb-1 ${line.startsWith('>') ? 'text-primary' : line.startsWith('[Error]') ? 'text-destructive' : 'text-muted-foreground'}`}
          >
            {line}
          </div>
        ))}
      ...
      <form
        ...
        className="border-t border-border bg-muted/10 p-2 flex items-center gap-2"
      >
